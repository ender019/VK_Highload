...
Балансируется на граничных маршрутизаторах ДЦ с помощью ECMP + BGP.
...
| **Отказоустойчивость** | VIP (Virtual IP) назначается на активный узел. Второй (резервный) узел находится в режиме ожидания. |
...
567 Гбит/с ÷ 100 Гбит/с = 5,67 -> 6 серверов


С учётом резервирования по схеме **N × 2** (полное дублирование):

Итого L4 серверов = 6 × 2 = 12 серверов
...
| :--- | :--- | :--- | :--- |
| **L4** | 12 | CPU 8 Cores, RAM 32 ГБ, NIC 100GbE | N × 2 |
