...
| **Отказоустойчивость** | VIP (Virtual IP) назначается на активный узел. Один резервный узел находится в режиме ожидания.  |
...
С учётом резервирования по схеме **N + 1** (полное дублирование):

Итого L4 серверов = 8 + 1 = 9 серверов
...
| **L4 (LVS)**    | 9          | CPU 8 Cores, RAM 32 ГБ, NIC 100GbE  | N + 1  |
| **L7 (nginx)**  | 9          | [8] | N + 1  |
...
flowchart TB
    ...
    subgraph L4_Load_Balancers [L4 Load Balancers: LVS DR Mode]
        LVS1[LVS 1]
        LVS2[LVS 2]
        LVS3[LVS 3]
        LVS4[LVS 4]
    ...
    %% ECMP балансировка с BGP на LVS
    BGP1 --> LVS1
    BGP1 --> LVS2
    BGP1 --> LVS3
    BGP1 --> LVS4
    ...
    %% Обратный трафик DSR (Direct Server Return)
    Content -.->|DSR: Payload| Client1
    API -.->|DSR: JSON| Client1
```
...
5.  **Data Path:** Тяжелые данные (файлы) хранятся в MinIO. При скачивании файл проходит через Nginx (для шифрования), но «пролетает» мимо LVS на выход в интернет, что обеспечивает линейную масштабируемость системы.
...
| `object_storage`                                  | **MinIO** | дешёвое масштабируемое хранение бинарных данных |
...
| `object_storage`           | MinIO      | `hash_prefix`                | Равномерное распределение объектов по bucket-префиксам                                                          |
...
| MinIO      | Erasure Coding внутри кластера                                                                                 | Защита данных при отказе дисков или серверов                                      |
...
| MinIO      | Load balancing между storage-нодами | Объекты распределяются по нескольким storage-нодам кластера, что позволяет равномерно распределять нагрузку                                        |
...
| MinIO      | Блоки файлов пользователей                  | Versioning + периодические snapshot-бэкапы     | Восстановление удалённых или повреждённых объектов      |
...
| **MinIO** | Хранение бинарных данных файлов (chunks) | Масштабируемое объектное хранилище, совместимое с S3 API, высокая отказоустойчивость, возможность развертывания на собственной инфраструктуре |
...
| **minio-go**  | Клиент для работы с объектным хранилищем MinIO   | Поддержка multipart-загрузки и интеграция с S3-совместимым API  |
