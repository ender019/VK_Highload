...
2.  **L7 (Nginx):** Выполняет самую ресурсоемкую задачу — **SSL Termination** (расшифровка трафика). После расшифровки Nginx анализирует URL (например, `api.disk...` или `content.disk...`) и проксирует запрос на соответствующий бэкенд.  **Резервирование:** 
...
4.  **Data Path:** Тяжелые данные (файлы) хранятся в SeaweedFS. При скачивании файл проходит через Nginx (для шифрования), но «пролетает» мимо LVS на выход в интернет, что обеспечивает линейную масштабируемость системы.
...
Все сервисы развёртываются в одном дата-центре (DC1, Москва) по модели собственного владения оборудованием (Colocation). Stateless-сервисы (Go-микросервисы) работают в Kubernetes; stateful-компоненты (БД, хранилище) — на выделенных физических серверах (Bare Metal) для исключения оверхеда на виртуализацию сети и дисков.

## 11.1 Ресурсные требования

Количество ядер CPU для микросервисов рассчитывается по формуле:
`Количество ядер CPU = Пиковая нагрузка / Норма нагрузки на 1 ядро`

### Нормы производительности
| Характер нагрузки | Норма на 1 ядро |
| :--- | :--- |
| **API API** (Auth, Metadata, Polling) | 3 000 RPS |
| **Content Service** (Hashing, kTLS, IO) | 2.5 Гбит/с |
| **Public/Static API** | 8 000 RPS |

### Сводная таблица ресурсов (Пиковая нагрузка)
| Сервис | Нагрузка (Пик) | Расчет | CPU (ядер) | RAM |
| :--- | :--- | :--- | :--- | :--- |
| **api-service** | 87 898 RPS | 87 898 / 3 000 ≈ 30 → 4 × 8 | 32 | 64 ГБ |
| **content-service** | 567,8 Гбит/с | 567.8 / 2.5 ≈ 228 → 32 × 8 | 256 | 1024 ГБ |
| **public-service** | 10 000 RPS | 10 000 / 8 000 ≈ 2 → 2 × 4 | 8 | 16 ГБ |
| **static-service** | 5 000 RPS | 5 000 / 8 000 ≈ 1 → 2 × 2 | 4 | 8 ГБ |
| **Итого** | | | **300** | **1112 ГБ** |

## 11.2 Конфигурации и стоимость

| Тип сервера | Модель | Конфигурация | Cores | Количество | Цена (₽) | Стоимость (₽) | Аренда/мес (₽) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **K8s Node** | CyberServe SP1 | 2x AMD EPYC 7763 / 512GB / 2x100GbE | 128 | 5 | 2 200 000 | 11 000 000 | 250 000 |
| **LVS L4** | CyberServe E12 | Xeon E-2414 / 32GB / 100GbE NIC | 4 | 16 | 345 000 | 5 520 000 | 120 000 |
| **Nginx L7** | CyberServe SP1 | Xeon Silver 4514Y / 128GB / 100GbE NIC | 16 | 20 | 549 000 | 10 980 000 | 244 000 |
| **PostgreSQL** | CyberServe SP1 | Xeon Gold 6426Y / 512GB / 4x4TB NVMe | 32 | 100 | 1 120 000 | 112 000 000 | 2 420 000 |
| **ScyllaDB** | CyberServe SP2 | 2x Xeon Gold 6426Y / 1TB / 8x4TB NVMe | 64 | 15 | 1 850 000 | 27 750 000 | 620 000 |
| **Redis** | CyberServe SP1 | Xeon Silver 4514Y / 256GB / 2x1TB NVMe | 16 | 12 | 680 000 | 8 160 000 | 180 000 |
| **Storage Node**| StorageServer | AMD EPYC 7443P / 256GB / 60x20TB HDD | 24 | 800 | 2 850 000 | 2 280 000 000 | 44 000 000 |
| **SW Master** | CyberServe E12 | Xeon E-2414 / 64GB / 2x1TB NVMe | 4 | 3 | 390 000 | 1 170 000 | 30 000 |
| **Итого** | | | | **971** | | **2 456 580 000** | **47 864 000** |

## 11.3 Аллокация в Kubernetes

| Сервис | Реплики | CPU Request | RAM Request | Назначение |
| :--- | :--- | :--- | :--- | :--- |
| **api-service** | 4 | 8 | 16 ГБ | Метаданные, Auth, Polling |
| **content-service** | 32 | 8 | 32 ГБ | Загрузка и выдача чанков |
| **public-service** | 2 | 4 | 8 ГБ | Проверка публичных ссылок |
| **static-service** | 2 | 2 | 4 ГБ | Раздача Web UI ассетов |

Примечание: CPU limits установлены равными CPU requests — задержки из-за throttling недопустимы для сервиса облачного хранения. RAM limits удвоены (x2) для защиты от OOM при пиковых нагрузках на сериализацию метаданных.

Для всех Go-сервисов устанавливается переменная окружения `GOMEMLIMIT` на уровне 90% от RAM limit контейнера. Это заставляет сборщик мусора (GC) Go работать более агрессивно при приближении к лимиту памяти контейнера, предотвращая принудительное завершение процесса ОС через OOM Killer.

## 11.4 Обоснование масштабирования
1. **SeaweedFS Volume:** Использование 800 серверов в конфигурации 60 HDD по 20 ТБ обеспечивает суммарную физическую емкость **960 ПБ**. Это покрывает расчетный объем данных пользователей (939 ПБ) с минимальным запасом. Для оптимизации чтения с HDD используется 256 ГБ RAM на ноду под Page Cache.
2. **PostgreSQL:** Для хранения метаданных 46.5 млрд объектов развернуто 50 шардов (100 серверов M+R). Это гарантирует, что индексы каждого шарда (объемом ~250-300 ГБ) полностью помещаются в RAM (512 ГБ), обеспечивая мгновенный отклик при навигации по папкам.
3. **ScyllaDB:** Кластер из 15 мощных нод (RF=3) используется для высоконагруженного Sync Log и глобального реестра чанков (дедупликация). 1 ТБ RAM на ноду позволяет эффективно кэшировать Bloom-фильтры для 31 млрд уникальных хешей.
4. **Kubernetes:** Суммарно Go-сервисы потребляют 300 ядер. С учетом 20% накладных расходов оркестрации требуется ~360 ядер. Пять серверов AMD EPYC (640 ядер) обеспечивают достаточный запас для бесперебойной работы и фоновых процессов.
5. **Балансировка:** 16 балансировщиков LVS работают в режиме **Direct Server Return (DSR)**, обрабатывая только входящие пакеты. Исходящий трафик (567,8 Гбит/с) уходит пользователям напрямую с узлов Content Service и Storage нод, что исключает бутылочное горлышко на сетевом уровне балансировки.
